...
from ..bindsnet_bad_zeros.models import DiehlAndCook2015
...
def supervised_mnist(seed, n_neurons, n_train, n_test, n_clamp, exc, inh, time, dt, intensity, update_interval, train, plot, directory, gpu):
    start_qa = clock.time()
    ...
    for (i, datum) in pbar:
        ...
        if plot:
            ...
            if i == n_train:
                perf_ax, perf_fig = plot_performance(accuracy, update_interval=update_interval, ax=perf_ax,
                                                     fig=perf_fig)
    ...
    print("Wall clock time taken BindsNET_Bad_Zeros: %fs." % elapsed_qa)

    return accuracy, elapsed_qa
